    ...
    stages {
        ...
        stage ('Deploy: Dev') {
            ...
            environment{
                ...
                ALPHANUMBERIC_BRANCH_NAME = "${env.BRANCH_NAME}".replaceAll("[^A-Za-z0-9]", "")
            }
            steps {
...
chmod +x deploy.sh
bash -x deploy.sh $MY_REPO $ARCHIVE $VERSION $ART_URL $ART_USR $ART_PWD $HDFS_USR $ALPHANUMBERIC_BRANCH_NAME
DEPLOY'''
                        }
                    }
                }
